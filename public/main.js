...
class AudioConference {
    ...
    setupWebRTCSignaling() {
        this.socket.on('offer', async ({ offerer, description }) => {
            try {
                let peerConnection = this.peers.get(offerer);
                ...
                peerConnection = this.createPeerConnection(offerer);
                ...
                // Send answer only if we're still connected
        ...
        this.socket.on('answer', async ({ answerer, description }) => {
            try {
                ...
                if (peerConnection.signalingState === "have-local-offer" && 
                    ...
                    console.warn(
                        "Skipping duplicate answer for", answerer,
        ...
        this.socket.on('ice-candidate', async ({ sender, candidate }) => {
            try {
                ...
                // Only add ice candidate if we have both local and remote descriptions
                if (peerConnection.remoteDescription && peerConnection.localDescription) {
                    await peerConnection.addIceCandidate(new RTCIceCandidate(candidate));
                } else {
                    console.warn('Dropped ICE candidate - not ready:', 
                        'Remote desc:', !!peerConnection.remoteDescription,
                        'Local desc:', !!peerConnection.localDescription
                    );
                }
            } catch (error) {
    ...
    createPeerConnection(userId) {
        ...
        peerConnection.onsignalingstatechange = () => {
            console.log(`Signaling state for peer ${userId}:`, peerConnection.signalingState);
        };
    ...
    async connectToNewUser(userId) {
        try {
            ...
            // Send offer only if we're still in a valid state
            if (peerConnection.connectionState !== 'closed') {
